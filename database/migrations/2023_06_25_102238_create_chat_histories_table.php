...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('chat_histories', function (Blueprint $table) {
            ...
            $table->float('temperature', 3, 2)->nullable()->comment('チャットのランダム性を表す値:0.0~2.0: 使用するサンプリング温度は0から2の間。0.8のような高い値は出力をよりランダムにし、0.2のような低い値は出力をより集中して決定論的にする。一般的には、この値かtop_pを変更することを推奨しますが、両方は推奨しません。');
            ...
            $table->float('presence_penalty', 3, 2)->nullable()->comment('以前と似た内容に対するペナルティ値:-2.0~2.0 の間の数値。正の値は、新しいトークンがこれまでにテキストに現れたかどうかに基づいてペナルティを課し、モデルが新しいトピックについて話す可能性を高める。');
            $table->float('frequency_penalty', 3, 2)->nullable()->comment('繰り返しに対するペナルティ値:-2.0~2.0の間の数値。正の値は、新しいトークンに、これまでのテキストにおける既存の頻度に基づいてペナルティを与え、モデルが同じ行を逐語的に繰り返す可能性を低下させる。');
